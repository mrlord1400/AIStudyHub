...
USE ai_study_hub;
-- New table added to store extracted text content from user documents.
-- Purpose: When the AI chatbot requests to view a document's content (VIEW/[document name] flow),
-- the system queries this table to retrieve the pre-extracted text and sends it to the AI for analysis,
-- instead of only sending metadata as before.
-- ON DELETE CASCADE on the foreign key ensures that when a document is deleted,
-- its extracted text is automatically deleted too — no changes needed to any existing Java delete logic.
GO
